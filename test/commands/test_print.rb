...
class TestPrint < Minitest::Test
  ...
  def test_html_renders_in_browser
    WebMock.disable_net_connect!
    stub_request(:get, 'https://yegor256.github.io/judges/assets/index.css').to_return(body: 'body { font-family: Arial; }')
    ...
    assert data_rows.size >= 1, 'Table should have at least one data row'
    
    # Verify table structure is semantically correct for accessibility
    header_cells = thead.css('th')
    assert header_cells.size >= 3, 'Table should have header cells for each column'
    ...
    if chrome_available? && we_are_online
      begin
